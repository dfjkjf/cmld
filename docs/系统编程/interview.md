...
| 特性  | 进程 | 线程 |
|:---|:---|:---|
| 定义 | 操作系统进行资源分配和调度的独立单位 | 进程中的执行流，CPU执行的基本单位 |
...
2. 进程是应用程序的执行副本，是操作系统进行资源分配的基本单位。线程是是CPU执行的基本单位，它是比进程更小的能独立运行的基本单位，并且线程自己不拥有系统资源，只拥有一点儿在运行中必不可少的资源，但它可以与同属一个进程的其他线程共享进程所拥有的全部资源。
