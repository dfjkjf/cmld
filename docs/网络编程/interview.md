...
- **网络层（Internet Layer）**：
   - 这一层包括了IP协议和其他相关的协议，如ICMP（互联网控制消息协议）、IGMP（互联网组管理协议）、ARP（地址解析协议）和NDP（邻居发现协议）。这些协议各自承担着不同的功能，共同确保网络通信的顺利进行。以下是这些协议的简要解释：
...
3. 例如，当一个数据包从应用层发送到传输层时，它会被封装成TCP或UDP数据段，然后传递给网络层。网络层将数据段封装成IP数据包，并添加源IP地址和目的IP地址。如果需要，它还会使用ICMP或IGMP发送控制消息。最后，IP数据包被封装成链路层的数据帧，并发送到物理媒介上。到达目的地后，数据帧被解封装，IP数据包被提取，然后ARP或NDP用于解析MAC地址，最后数据包被传递到传输层，再由传输层传递给应用层。
1. IP地址是网络层地址，用于路由数据包；MAC地址是链路层地址，用于局域网内设备间的通信。
1. 子网掩码用于将IP地址划分为网络地址和主机地址，帮助确定数据包发送的目的地。CIDR块使用斜线记法（例如，192.168.1.0/24），其中“/24”表示前24位是网络部分。
1. DNS是域名系统，它将域名转换为IP地址，通过递归或迭代查询方式工作。
1. 需要内网IP的原因：地址管理，网络隔离，成本。
...
2. TCP提供可靠的、面向连接的服务，UDP提供不可靠的、无连接的服务。
   TCP会将应用层下来的数据分片，UDP则视为对立的报文。
3. 流量控制是作用于发送方，调整发送速度，通过滑动窗口实现。拥塞控制是针对发送接收双方间的网络，让发送方更多地利用网络带宽，不能在网络拥塞的情况下还拼命发送数据。包括慢启动、拥塞避免、快速重传和快速恢复等算法。
...
4. TCP的序号用于标识数据包的顺序，确认序号用于告诉发送方哪些数据包已经被接收，在指定时间内没有发送方收到确认序号就说明丢包了。
